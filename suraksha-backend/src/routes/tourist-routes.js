...
function registerTouristRoutes(app, context) {
  app.post('/api/registerTourist', (req, res) => {
    ...
    if (name.length < context.config.validation.minimumNameLength) errors.push('Name is too short.');
    if (!isValidPhone(phone)) errors.push('Phone number must be valid.');
    if (identityNumber.length < context.config.validation.minimumIdentityLength) errors.push('Identity number is required.');
    for (const contact of emergencyContacts) {
      if (contact.phone && !isValidPhone(contact.phone)) errors.push(`Emergency contact phone is invalid: ${contact.phone}`);
    }
    if (errors.length) return sendError(res, 400, 'Tourist registration validation failed.', errors);
    ...
    const blockchainHash = hashPayload({ touristId, identityNumber, name, phone, registeredAt, nonce: cryptoNonce() });
    ...
    const block = addBlock(data, context.config, 'TOURIST_REGISTERED', { touristId, blockchainHash });
  ...
  app.get('/api/tourists/:id', (req, res) => {
    ...
    sendSuccess(res, publicTourist(tourist, context.config));
  });
